...
-- Description:	ѕолучение статистики по продажам за текущий день по товарам
-- =============================================
ALTER PROCEDURE [OnlineStore].[GetRealizTovarForEan]	
	 @listEAN nvarchar(max)
AS
BEGIN
	SET NOCOUNT ON;
...
--‘ормирование переменной текущей даты
...
--—оздание временной таблицы дл€ товаров
...
select 
	trim(e.value collate Cyrillic_General_CS_AS) as ean,cast(0.00 as numeric(16,3)) as  netto,g.price/100.00 as price
from 
	dbo.StringToTable(@listEAN,',') e
	inner join dbo.goods_updates g on trim(g.ean) = e.value collate Cyrillic_General_CS_AS and g.ActualRow = 1 
) as a
group by a.ean,a.price

...
-- Description:	Получение остатков на текущую дату
-- =============================================
ALTER PROCEDURE [OnlineStore].[GetOstOrderNow]	
	 @id_tOrders int = 1118
AS
BEGIN
	SET NOCOUNT ON;
...
--Расчёт средней реализации 
CREATE TABLE #tableMidleRealiz (id_tovar int, netto numeric(13,2))

INSERT INTO #tableMidleRealiz
select a.id_tovar,sum(a.netto)/count(a.drealiz)  as resultNetto
from(
select 
	r.id_tovar,sum(netto) as netto,drealiz
from 
	#tableTovar t
		inner join  dbo.j_realiz r on r.id_tovar = t.id_tovar
where
	DATEADD(day,-60,@dateEnd)<=drealiz and drealiz<=@dateEnd
group by r.id_tovar,r.drealiz) as a
group by a.id_tovar
...
select 
	...
	isnull(tmr.netto,0.00) as midRealiz
from 
	#tableTovar t
		...
		left join #tableMidleRealiz tmr on tmr.id_tovar = t.id_tovar

...
-- Description:	Получение сумма за период с учётом rcena
-- =============================================
...
BEGIN
	...
	DECLARE @DateCheck datetime, @CheckNumber int, @KassNumber int 
	...
	BEGIN	
			...
			select @resultSum=@resultSum+sum(k.cnt*isnull(r.rcena,0)) from @Table k inner join dbo.s_tovar t on ltrim(rtrim(t.ean)) = k.ean inner join @rCena r on r.id_tovar = t.id

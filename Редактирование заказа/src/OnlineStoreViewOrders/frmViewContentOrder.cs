...
namespace OnlineStoreViewOrders
{
    public partial class frmViewContentOrder : Form
    {
        ...
        private void dgvOrder_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
        {
            ...
            if (new List<int>() { 1, 2 }.Contains(id_Status))
        ...
        private async void SaveData()
        {
            ...
            foreach (DataRow row in dtGoods.Rows)
            {
                ...
                if (row["id"].ToString()=="")
                {
                    Logging.Comment($"Новый товар: id: {row["id_tovar"].ToString()}, ean: {row["ean"].ToString()}, " +
                       $"Наименование: {row["nameTovar"].ToString()}, Цена: {row["Price"].ToString()}, Количество: {row["Netto"].ToString()}," +
                       $"Сумма: {row["sumTovar"].ToString()}");
                    Config.connect.Set_Order(int.Parse(row["id_tOrders"].ToString()), int.Parse(row["id_Tovar"].ToString()),
                                            "", int.Parse(row["Position"].ToString()), decimal.Parse(row["Netto"].ToString().Replace('.', ',')), decimal.Parse(row["Price"].ToString().Replace('.', ',')));
                    isEdit = true;
                }
                else
                {
                    var rowOld = dtGoodsOld.AsEnumerable().Where(r => r.Field<int>("id") == (int)row["id"]);
                    if (rowOld.Count()>0 && (decimal)rowOld.First()["Netto"]!= (decimal) row["Netto"])         
                    {
                        Logging.Comment($"id товара: {row["id_tovar"].ToString()}, ean: {row["ean"].ToString()}, " +
                           $"Наименование: {row["nameTovar"].ToString()}, Цена: {row["Price"].ToString()}");
                        Logging.VariableChange($"Количество:", row["Netto"], rowOld.First()["Netto"]);
                        Logging.VariableChange("Сумма", row["sumTovar"], rowOld.First()["sumTovar"]);
                        Config.connect.Set_Order(int.Parse(row["id_tOrders"].ToString()), int.Parse(row["id_Tovar"].ToString()),
                            "", int.Parse(row["Position"].ToString()), (row["Netto"].ToString().Trim().Length > 0 ? decimal.Parse(row["Netto"].ToString().Replace('.', ',')) : 0.000M), decimal.Parse(row["Price"].ToString().Replace('.', ',')));
                        isEdit = true;
                    }
                }

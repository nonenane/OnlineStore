...
public class NetworkShare : IDisposable
{
    ...
    public NetworkShare(bool init)
    {
        if (init)
        {
            ...
            DataTable dt = _proc.getSettings("lnln").Result;
            this.server = dt.Rows[0]["value"].ToString();

            dt = _proc.getSettings("lnlg").Result;
            this.login = dt.Rows[0]["value"].ToString();

            dt = _proc.getSettings("lnps").Result;
            this.password = dt.Rows[0]["value"].ToString();

            //this.server = dt.AsEnumerable().Where(r => r.Field<string>("id_value") == "lnln").CopyToDataTable().Rows[0]["value"].ToString();
            //this.login = dt.AsEnumerable().Where(r => r.Field<string>("id_value") == "lnlg").CopyToDataTable().Rows[0]["value"].ToString();
            //this.password = dt.AsEnumerable().Where(r => r.Field<string>("id_value") == "lnps").CopyToDataTable().Rows[0]["value"].ToString();
        }       
    }
    ...
    public bool CopyFile(string fullname)
    {
        try
        {
            ConnectToShare();
            ...
            {
                MessageBox.Show(Config.centralText("Программа не может получить доступ к сетевой папке!\nДобавление файла невозможно обратитесь в ОЭЭС.\n"),"Добавление изображения товара",MessageBoxButtons.OK,MessageBoxIcon.Warning);
                return false;
            }
            File.Copy(fullname, server + "\\" + Path.GetFileName(fullname), true);
    ...
    public bool RemoveFile(string fullname)
    {
        try
        {
            ...
            File.Delete(server + "\\" + Path.GetFileName(fullname));
    ...
    public byte[] GetFileBytes(string nameFile)
    {
        byte[] file = null;
        ConnectToShare();
